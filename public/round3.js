(() => {
  ...
  function showFatalError(message) {
    ...
    const app = document.getElementById('round3-app');
    if (app) app.prepend(banner);
    else document.body.prepend(banner);
  }

  const DATA = window.ROUND3_DATA;
  if (!DATA) {
    showFatalError('Round 3 data failed to load. Run scripts/build-round3-web.ps1, then hard-refresh this page.');
    return;
  }
  if (!Array.isArray(DATA.sections) || !DATA.sections.length) {
    showFatalError('Round 3 data is empty or malformed. Re-run scripts/build-round3-web.ps1.');
    return;
  }
  if (!DATA.topics?.['1']?.round3Text) {
    showFatalError('Round 3 prayer text missing from data. Re-run scripts/build-compiled-prayers-round3.ps1 and build-round3-web.ps1, then hard-refresh.');
    return;
  }
  ...
  const els = {
    ...
    progress: document.getElementById('round3-progress'),
    phasePrayers: document.getElementById('phase-prayers'),
    sidebarSections: document.getElementById('round3-sidebar-sections'),
  ...
  function renderCurrentTopic() {
    ...
    if (!t) {
      ...
      els.prayerText.textContent = 'Prayer text missing for this topic. Re-run scripts/build-round3-web.ps1.';
      return;
    }
