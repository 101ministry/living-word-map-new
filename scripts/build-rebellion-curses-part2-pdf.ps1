# Build Rebellion + Desertion = Curses Part 2 (modern medical correlates) HTML + PDF

$ErrorActionPreference = 'Stop'
$Root = Split-Path $PSScriptRoot -Parent
$OutHtml = Join-Path $Root 'data/Rebellion-Desertion-Curses-Part2.html'
$OutPdf  = Join-Path $Root 'data/Rebellion-Desertion-Curses-Part2.pdf'
$OutPdfTitled = Join-Path $Root 'data/Rebellion + Desertion = Curses (Part 2).pdf'
...
if (-not $browser) {
    Write-Warning 'Chrome/Edge not found - HTML only.'
    exit 0
}
...
if (Test-Path $OutPdf) {
    Copy-Item $OutPdf $OutPdfTitled -Force
    Write-Host "Wrote PDF: $OutPdf ($((Get-Item $OutPdf).Length) bytes)"
    Write-Host "Titled copy: $OutPdfTitled"
} else {
